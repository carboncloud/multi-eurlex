...
class SampleGenerator(tf.keras.utils.Sequence):
    ...
    def vectorize_data(self, documents, special_token=""):
        x = np.asarray(
            self.tokenizer.batch_encode_plus(
                [special_token + document["text"] for document in documents],
    ...
    def __getitem__(self, index):
        ...
        if not self.multilingual_train:
            valid_docs = [
                document for document in documents if document["text"] is not None
            ]
        else:
            valid_docs = [
                document for document in documents if document["text"] is not None
        ...
        if len(valid_docs) == 0:
            return [], []
        x_batch, y_batch = self.vectorize_data(
            valid_docs,
            special_token=(
                "<extra_id_0>" if "mt5" in self.tokenizer.name_or_path else ""
            ),
        )
