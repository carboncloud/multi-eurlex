...
class TFBertSelfOutput(tf.keras.layers.Layer):
    def __init__(
        self,
        pretrained_self_dense: tf.keras.layers.EinsumDense,
        pretrained_self_ln: tf.keras.layers.LayerNormalization,
        config: AutoConfig,
        bottleneck_size: int,
        **kwargs,
    ):
...
class TFBertOutput(tf.keras.layers.Layer):
    def __init__(
        self,
        pretrained_out_dense: tf.keras.layers.EinsumDense,
        pretrained_out_ln: tf.keras.layers.LayerNormalization,
        config: AutoConfig,
        bottleneck_size: int,
        **kwargs,
    ):
        ...
        if num_frozen_layers or use_adapters or use_ln:
            if self.bert.config.model_type in ["xlm-roberta", "roberta"]:
                ...
                raise NotImplementedError(
                    f"Model type {self.bert.config.model_type} is not supported for adaptation"
                )

    def adapt_model_bert(
        self,
        num_frozen_layers=None,
        bottleneck_size=256,
        use_adapters=False,
        ...
        if num_frozen_layers:
            ...
            for i in range(num_frozen_layers):
                self.bert.bert.encoder.layer[i].attention.self_attention.trainable = (
                    False
                )
                self.bert.bert.encoder.layer[
                    i
                ].attention.dense_output.dense.trainable = False
                if not (use_adapters or use_ln):
                    self.bert.bert.encoder.layer[
                        i
                    ].attention.dense_output.LayerNorm.trainable = False
                ...
                if not (use_adapters or use_ln):
                    self.bert.bert.encoder.layer[i].bert_output.LayerNorm.trainable = (
                        False
                    )

    def adapt_model_roberta(
        self,
        num_frozen_layers=None,
        bottleneck_size=256,
        use_adapters=False,
        ...
        if use_adapters:
            ...
            for i in range(self.bert.roberta.config.num_hidden_layers):
                # instantiate
                self.bert.roberta.encoder.layer[i].attention.dense_output = (
                    TFBertSelfOutput(
                        self.bert.roberta.encoder.layer[i].attention.dense_output.dense,
                        self.bert.roberta.encoder.layer[
                            i
                        ].attention.dense_output.LayerNorm,
        ...
        if num_frozen_layers:
            ...
            for i in range(num_frozen_layers):
                self.bert.roberta.encoder.layer[
                    i
                ].attention.self_attention.trainable = False
                self.bert.roberta.encoder.layer[
                    i
                ].attention.dense_output.dense.trainable = False
                if not (use_adapters or use_ln):
                ...
                if not (use_adapters or use_ln):
                    self.bert.roberta.encoder.layer[
                        i
                    ].bert_output.LayerNorm.trainable = False
