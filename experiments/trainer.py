...
def train(
    ...
    LOGGER.info("----------------------------------------")
    LOGGER.info(
        f"     Training Language: {train_lang if not multilingual_train else train_langs}"
    )
    ...
    train_generator = SampleGenerator(
        ...
        shuffle=True,
        multilingual_train=multilingual_train,
        max_document_length=max_document_length,
    )

    LOGGER.info(f"{len(val_dataset)} documents will be used for development")
    ...
    if monitor == "val_loss":
        ...
        for n_epoch, _ in enumerate(history.history["loss"]):
            LOGGER.info(
                f'EPOCH #{n_epoch + 1:<2}: \t{history.history["loss"][n_epoch]:.5f}'
    ...
    dev_generator = SampleGenerator(
        dataset=eval_dataset["validation"][
            : eval_samples if eval_samples else len(eval_dataset["validation"])
        ],
        label_index=label_index,
        lang=train_lang,
        bert_model_path=bert_path,
    ...
    for lang_code in eval_langs:
        ...
        n_documents = sum(
            [
                1
                for document in dev_generator.documents
                if document["text"][lang_code] is not None
            ]
        )
        ...
        for k in range(1, 6):
            scores += (
                f"NDCG@{k}: {mean_ndcg_score(y_true, y_pred, k=k)[0] * 100:2.2f}\t"
            )
        for k in range(1, 6):
    ...
    test_generator = SampleGenerator(
        dataset=eval_dataset["test"][
            : eval_samples if eval_samples else len(eval_dataset["test"])
        ],
        label_index=label_index,
        lang=train_lang,
        bert_model_path=bert_path,
    ...
    for lang_code in eval_langs:
        ...
        n_documents = sum(
            [
                1
                for document in test_generator.documents
                if document["text"][lang_code] is not None
            ]
        )
        y_true = np.zeros((n_documents, len(label_index)), dtype=np.float32)
    ...
    LOGGER.info(
        f'Training + Evaluation time: {time.strftime("%H:%M:%S", time.gmtime(time.time() - start_training_time))} sec\n'
    )
...
def r_precision(y_true, y_pred):
    positives = tf.reduce_sum(
        tf.cast(tf.equal(y_true, 1.0), dtype=tf.keras.backend.floatx()), axis=-1
    )
    y_true_ranked = tf.cast(
        tf.gather(y_true, tf.argsort(y_true, direction="DESCENDING"), batch_dims=1),
        dtype=tf.bool,
    )
    y_true_pred_ranked = tf.cast(
        tf.gather(y_true, tf.argsort(y_pred, direction="DESCENDING"), batch_dims=1),
        dtype=tf.bool,
    )
    relevant = tf.reduce_sum(
        tf.cast(
            tf.logical_and(y_true_ranked, y_true_pred_ranked),
